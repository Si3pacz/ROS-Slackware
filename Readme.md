...
Note: ROS still primarily uses Python 2.7 and you should use that version of python for installation.
...
Although this package is available on SlackBuilds.org we still need to manually install beforehand due 
...
Open `src/python_qt_binding/cmake/sip_configure.py` with your favourite editor. Find the
lines:
```
env['QT_SELECT'] = '5'
...
Open `src/geometric_shapes/CMakeLists.txt` with your favourite editor.
Find the line `find_package(Eigen3 REQUIRED)` and substitute it with
```
